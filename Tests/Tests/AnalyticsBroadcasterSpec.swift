    ...
    override func spec() {
        describe("sendEventName") {
            ...
            beforeEach {
                ...
                observer = NotificationCenter.default.addObserver(forName: .rAnalyticsCustomEvent,
                                                                  object: nil,
                                                                  queue: OperationQueue()) { notification in
                    tracker?.trackEvent(name: NSNotification.Name.rAnalyticsCustomEvent.rawValue, parameters: notification.object as? [String: Any])
            ...
            it("should track sdkCustomEvent with eventName and eventData when an event is broadcasted with data") {
                ...
                expect(tracker?.eventName).toEventually(equal(NSNotification.Name.rAnalyticsCustomEvent.rawValue))
